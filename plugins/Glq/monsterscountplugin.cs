...
namespace Turbo.Plugins.glq
{
    ...
    public class MonstersCountPlugin : BasePlugin, IInGameTopPainter, IInGameWorldPainter, IKeyEventHandler
    {
        ...
        public void PaintTopInGame(ClipState clipState)
        {
            ...
            {
                textBuilder.AppendFormat("{0} Yards Mob Count: {1}", currentYard, monstersCount);
                textBuilder.AppendLine();
                if (EliteCount > 0) textBuilder.AppendFormat("Elite: {0}(Amount: {1})", EliteCount, ElitePackCount);
                if (RareMinionCount > 0) textBuilder.AppendFormat("Minions: {0}", RareMinionCount);
                textBuilder.AppendLine();
                textBuilder.AppendLine();
            }
            ...
            if (inRift)
            {
                ...
                if (totalMonsterRiftProgression > 0 && ShowTotalProgression)
                {
                    if (ShowTime && inGR)
                    {
                        textBuilder.AppendFormat("Progression: {0}% = {1}", totalMonsterRiftProgression.ToString("f2"), ValueToString((long)totalTime, ValueFormat.LongTime));
                    }
                    else
                    {
                        textBuilder.AppendFormat("Progression: {0}%", totalMonsterRiftProgression.ToString("f2"));
                    }
                    textBuilder.AppendLine();
                }
                ...
                if (TrashMonsterRiftProgression > 0 && ShowTrashProgression)
                {
                    if (ShowTime && inGR)
                    {
                        textBuilder.AppendFormat("Trash Progression: {0}% = {1}", TrashMonsterRiftProgression.ToString("f2"), ValueToString((long)TrashTime, ValueFormat.LongTime));
                    }
                    else
                    {
                        textBuilder.AppendFormat("Trash Progression: {0}%", TrashMonsterRiftProgression.ToString("f2"));
                    }
                    textBuilder.AppendLine();
                }
                if (EliteProgression > 0 && ShowEliteProgression)
                {
                    if (ShowTime && inGR)
                    {
                        textBuilder.AppendFormat("Elite Progression: {0}% = {1}", EliteProgression.ToString("f2"), ValueToString((long)EliteTime, ValueFormat.LongTime));
                    }
                    else
                    {
                        textBuilder.AppendFormat("Elite Progression: {0}%", EliteProgression.ToString("f2"));
                    }
                    textBuilder.AppendLine();
                }
