...
namespace Turbo.Plugins.RNN
{
	public class BuildsIconsAndCoe : BasePlugin, IInGameTopPainter, ICustomizer, INewAreaHandler
	{
		...
		public override void Load(IController hud)
        {
			...
			Ypor = 0.05f;				// To set the y coordinate of the icon
		...
		public void Customize()
		{
			...
			FontStacks = Hud.Render.CreateFont("tahoma", 9f * SizeMultiplier, 255, 255, 255, 0, true, false, 255, 0, 0, 0, true);
			...
			FontStacksRed = Hud.Render.CreateFont("tahoma", 9f * SizeMultiplier, 255, 255, 0, 0, true, false, 160, 0, 0, 0, true);
			...
			foreach(var player in players)
			{
				...
				if (player.HeroClassDefinition.HeroClass == HeroClass.Necromancer)
				{
					...
					else if (player.Powers.BuffIsActive(476587))
					{
						...
						skill = player.Powers.UsedNecromancerPowers.BoneArmor;  // 466857
						if (player.Powers.UsedSkills.Any(s => s.SnoPower.Sno == Hud.Sno.SnoPowers.Necromancer_BoneArmor.Sno ))  // 466857
						{
							if (skill.Buff != null)
							{
								DrawIconBuff(x,y,SizeIconWidth,SizeIconHeight,skill.SnoPower.NormalIconTextureId,skill.Buff.IconCounts[0],skill.Buff.TimeLeftSeconds[0], skill.Buff.TimeElapsedSeconds[0] );
							...
							if (total > 0)
							{
								...
								foreach (var actor in actors)
								{
									...
									else
									{
										...
										FontDefault.DrawText(layout, x + ((SizeIconWidth - (float)Math.Ceiling(layout.Metrics.Width))/7.0f), y + (layout.Metrics.Height * 0.80f * (c - 1)) );
									}
								}
							}
